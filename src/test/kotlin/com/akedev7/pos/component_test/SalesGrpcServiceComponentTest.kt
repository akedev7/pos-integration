...
class SalesGrpcServiceComponentTest : ComponentTestBase() {
    ...
    private fun prepareData() {
        ...
        insertPayment(1, BigDecimal("50.00"), BigDecimal("100.00"), BigDecimal("0.50"), "PAYPAL", baseTime.plusMinutes(15))
        insertPayment(2, BigDecimal("75.00"), BigDecimal("100.00"), BigDecimal("0.75"), "CREDIT_CARD", baseTime.plusMinutes(30))

        // Hour 2 - 2 payments (1 from customer 3, 1 from customer 1)
        insertPayment(3, BigDecimal("200.00"), BigDecimal("100.00"), BigDecimal("2.00"), "DEBIT_CARD", baseTime.plusHours(1))
        insertPayment(1, BigDecimal("30.00"), BigDecimal("100.00"), BigDecimal("0.30"), "CREDIT_CARD", baseTime.plusHours(1).plusMinutes(45))

        // Hour 3 - 1 payment from customer 4
        insertPayment(4, BigDecimal("150.00"), BigDecimal("100.00"), BigDecimal("1.50"), "PAYPAL", baseTime.plusHours(2))
    }
    ...
    fun `should return sales data aggregated by hour`(): Unit = runBlocking {
        ...
        val response = grpcStub.getSalesData(
            Payment.SalesDataRequest.newBuilder()
                .setStartDateTime(
        ...
        assertThat(hour1.points).isEqualTo(Decimal.newBuilder().setValue("300.00").build()) // Assuming 1 point per 1 currency unit
